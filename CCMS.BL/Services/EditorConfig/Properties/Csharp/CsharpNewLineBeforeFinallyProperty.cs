...
{
    /// <summary>Determines if "finally" statement is on newline.</summary>
    public class CsharpNewLineBeforeFinallyProperty : IProperty, ICheckable, IImportable
    {
        public string Name => this.ToName();
        public string Description => "Determines if \"finally\" statement is on newline.";
        ...
        public CheckResult Check(string value, FileInfo file)
        {
            ...
            var clausules = root.DescendantNodes().OfType<FinallyClauseSyntax>();
            ...
            foreach (var clausule in clausules)
            {
                if (value == "true" && !IsEndOfLineBefore(clausule))
                    failedLines.Add(clausule.FinallyKeyword.GetLocation().GetLineSpan().StartLinePosition.Line);
        ...
        private bool IsEndOfLineBefore(FinallyClauseSyntax clause)
        {
            return clause.FinallyKeyword.LeadingTrivia.Any(t => t.Kind() is SyntaxKind.EndOfLineTrivia) ||
                   clause.FinallyKeyword.GetPreviousToken().TrailingTrivia
