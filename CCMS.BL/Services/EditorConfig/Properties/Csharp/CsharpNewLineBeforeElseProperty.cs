...
{
    /// <summary>Determines if "else" statement is on newline.</summary>
    public class CsharpNewLineBeforeElseProperty : IProperty, ICheckable, IImportable
    {
        public string Name => this.ToName();
        public string Description => "Determines if \"else\" statement is on newline.";
        ...
        public CheckResult Check(string value, FileInfo file)
        {
            ...
            var clausules = root.DescendantNodes().OfType<ElseClauseSyntax>();
            ...
            foreach (var clausule in clausules)
            {
                if (value == "true" && !IsEndOfLineBefore(clausule))
                    failedLines.Add(clausule.ElseKeyword.GetLocation().GetLineSpan().StartLinePosition.Line);
        ...
        private bool IsEndOfLineBefore(ElseClauseSyntax clause)
        {
            return clause.ElseKeyword.LeadingTrivia.Any(t => t.Kind() is SyntaxKind.EndOfLineTrivia) ||
                   clause.ElseKeyword.GetPreviousToken().TrailingTrivia
